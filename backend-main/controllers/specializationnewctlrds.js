...
exports.searchStudents = async (req, res) => {
  try {
    ...
    filters.forEach((filter) => {
      ...
      if (!aliases) return;
      const value = ["name", "email", "regno", "rollno"].includes(field) ? new RegExp(text(filter.value), "i") : text(filter.value);
      if (aliases.length === 1) query[aliases[0]] = value;
      else query.$and = [...(query.$and || []), { $or: aliases.map((alias) => ({ [alias]: value })) }];
    });
    const data = await User.find(query).select("name email phone regno rollno academicyear admissionyear regulation program programcode semester section specialization1 specialization2 colid").sort({ rollno: 1, name: 1 }).lean();
    res.json({ success: true, data });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
