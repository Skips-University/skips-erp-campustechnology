...
const sameValue = (left, right) => text(left).toLowerCase() === text(right).toLowerCase();
const studentCanTakeExam = (exam, student, classGroups = []) => {
  if (text(exam.section) && !sameValue(exam.section, student.section)) return false;
  if (text(exam.specialization) && ![student.specialization1, student.specialization2].some((value) => sameValue(value, exam.specialization))) return false;
  if (text(exam.classgroup)) {
    const matchesGroup = classGroups.some((group) => sameValue(group.groupname, exam.classgroup)
      && (!text(exam.coursecode) || sameValue(group.coursecode, exam.coursecode))
      && (!text(exam.programcode) || sameValue(group.programcode, exam.programcode)));
...
exports.options = async (req, res) => {
  try {
    ...
    const [courses, users, classGroups, specializations, ollama, ...responseValuesList] = await Promise.all([
      RegulationCourseMap.find({ colid }).select("academicyear program programcode course coursecode semester regulation").lean(),
      User.find({ colid, role: /^Student$/i }).select("name email regno academicyear program programcode semester section specialization1 specialization2").limit(2000).lean(),
      NepLmsClassGroup.find({ colid }).select("academicyear regulation program programcode semester course coursecode groupname section").lean(),
      SpecializationNew.find({ colid, status: { $not: /^Inactive$/i } }).select("academicyear regulation program programcode semester specialization").lean(),
...
exports.saveExam = async (req, res) => {
  try {
    ...
    let row;
    if (req.body.id) {
      const existing = await OnlineExam.findOne({ _id: req.body.id, colid: data.colid });
      if (!existing) return res.status(404).json({ success: false, message: "Exam not found" });
      const wasPublished = existing.hasbeenpublished || /^Published$/i.test(existing.status);
      if (wasPublished && /^Draft$/i.test(data.status)) {
        return res.status(400).json({ success: false, message: "A published exam cannot be changed back to Draft" });
      }
      data.hasbeenpublished = wasPublished || /^Published$/i.test(data.status);
      row = await OnlineExam.findOneAndUpdate({ _id: req.body.id, colid: data.colid }, data, { new: true });
    } else {
      data.hasbeenpublished = /^Published$/i.test(data.status);
      row = await OnlineExam.create(data);
    }
...
exports.saveSection = async (req, res) => {
  try {
    ...
    if (!/^Draft$/i.test(exam.status)) return res.status(400).json({ success: false, message: "Sections can be changed only while the exam is Draft" });
    const payload = {
...
exports.saveQuestion = async (req, res) => {
  try {
    const exam = await OnlineExam.findOne({ _id: req.body.examid, colid: num(req.body.colid) });
    if (!exam) return res.status(404).json({ success: false, message: "Exam not found" });
    if (!/^Draft$/i.test(exam.status)) return res.status(400).json({ success: false, message: "Questions can be changed only while the exam is Draft" });
    const section = exam.sections.id(req.body.sectionid);
...
exports.studentExams = async (req, res) => {
  try {
    ...
    const classGroups = await NepLmsClassGroup.find({ colid, regno: user.regno }).select("groupname coursecode programcode").lean();
    const eligibleExams = exams.filter((exam) => studentCanTakeExam(exam, user, classGroups));
    const attempts = await OnlineExamAttempt.find({ colid, regno, examid: { $in: eligibleExams.map((e) => e._id) } }).lean();
    const byExam = Object.fromEntries(attempts.map((a) => [String(a.examid), a]));
    res.json({ success: true, student: user, data: eligibleExams.map((exam) => ({ ...exam, attempt: byExam[String(exam._id)] || null, canStart: currentAllowed(exam) && !byExam[String(exam._id)]?.submittime })) });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message });
  }
};
...
exports.startAttempt = async (req, res) => {
  try {
    ...
    const classGroups = await NepLmsClassGroup.find({ colid: exam.colid, regno: student.regno }).select("groupname coursecode programcode").lean();
    if (!studentCanTakeExam(exam, student, classGroups)) return res.status(403).json({ success: false, message: "You are not eligible for this exam" });
